...
parser.add_argument('--extractSegments',       dest='extractSegments', action='store_true', help='Extract speaking segments after processing')
parser.add_argument('--segmentThreshold',      type=float, default=0.0,  help='Speaking score threshold for segment extraction')
...
def crop_video(args, track, cropFile):
	...
	# Keep the WAV file temporarily - it will be used by evaluate_network and then deleted
	return {'track':track, 'proc_track':dets, 'audioTmp':audioTmp}
...
def extract_speaking_segments(tracks, scores, args):
	"""Extract video segments where each person is speaking"""
	outputPath = os.path.join(args.savePath, 'speaking_segments')
	os.makedirs(outputPath, exist_ok=True)
	
	total_segments = 0
	min_frames = int(args.segmentMinDuration * 25)
	
	for track_idx, (score, track) in enumerate(zip(scores, tracks)):
		score_array = numpy.array(score)
		
		# Find speaking segments
		speaking = score_array > args.segmentThreshold
		segments = []
		start = None
		...
		if start is not None and (len(speaking) - start) >= min_frames:
		...
		for seg_idx, (start_frame, end_frame) in enumerate(segments):
			...
			output_file = os.path.join(outputPath, f'track_{track_idx:05d}_segment_{seg_idx:03d}.avi')
			output_wav = os.path.join(outputPath, f'track_{track_idx:05d}_segment_{seg_idx:03d}.wav')
			...
			# Extract audio segment (from WAV for better quality/speed)
			if os.path.exists(input_audio):
				cmd_wav = f"ffmpeg -y -i {input_audio} -ss {start_time:.3f} -t {duration:.3f} -c copy {output_wav} -loglevel panic"
			else:
				# Fallback: extract from video if WAV doesn't exist
	...
	# Clean up WAV files in pycrop directory after extraction (no longer needed)
	wav_files = glob.glob(os.path.join(args.pycropPath, '*.wav'))
	for wav_file in wav_files:
		os.remove(wav_file)
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Cleaned up temporary WAV files from %s \r\n" %args.pycropPath)
	
	sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + f" Extracted {total_segments} speaking segments to {outputPath}\r\n")
...
def main():
	...
	if os.path.exists(args.savePath):
		if os.path.isfile(args.savePath):
			...
			sys.stderr.write(f"Removed file with same name: {args.savePath}\n")
		else:
	...
	if args.evalCol == True:
		...
		if args.extractSegments:
			extract_speaking_segments(vidTracks, scores, args)
		else:
			# Clean up WAV files if not extracting segments
			wav_files = glob.glob("%s/*.wav"%args.pycropPath)
			for wav_file in wav_files:
				os.remove(wav_file)
			sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Cleaned up temporary WAV files from %s \r\n" %args.pycropPath)
