...
class talkNet(nn.Module):
    ...
    def train_network(self, loader, epoch, **kwargs):
        ...
        for num, (audioFeature, visualFeature, labels) in enumerate(loader, start=1):
            ...
            audioFeature = audioFeature[0].to(self.device)
            visualFeature = visualFeature[0].to(self.device)
    ...
    def evaluate_network(self, loader, evalCsvSave, evalOrig, **kwargs):
        ...
        for audioFeature, visualFeature, labels in tqdm.tqdm(loader):
            with torch.no_grad():
                ...
                predScores.extend(predScore[:,1].detach().cpu().numpy())
        ...
        cmd = (
            "python -O utils/get_ava_active_speaker_performance.py "
            "-g %s -p %s " % (evalOrig, evalCsvSave)
        )
        mAP = float(
            str(subprocess.run(cmd, shell=True, capture_output=True).stdout)
            .split(' ')[2][:5]
        )
        return mAP
